...
export const TLDR = ({ content, delay, id, triggerImage}) => {
  useEffect(() => {
    ...
    childArr.forEach((element, index) => {
      ...
      currDelay += element.classList.contains('link') || element.classList.contains('emphasis') ? 5*delay : delay;
    });
  });
  return (
            ...
            switch(block.type) {
                case 'text':
                    return block.content.split(" ").map((word, jndex) => {
                        return (<div key={`${word}-${jndex}`} className={styles.word}>{word}</div>)
                    })
                case 'emphasis':
                    return <div key={`${block.content}-${index}`} className='emphasis'>{block.content}</div>
                case 'link':
                    return <Link key={`${block.content}-${index}`} className='link' href={block.link}>{block.content}</Link>
            }
        })
      }
    </div>
