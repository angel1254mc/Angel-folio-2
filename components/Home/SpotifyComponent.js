...
const SpotifyComponent = () => {
  ...
  const mapArtists = useCallback((artists) => {
    return artists.map((artistObj, index) => {
        if (artists.length == 1 || (artists.length - 1) == index)
            return (
                <a href={artistObj?.external_urls?.spotify ?? ""} key={artistObj.id}>{artistObj.name}</a>
            )
        else (artists.length > 1 && index != (artists.length - 1))
            return (
                <a target="_blank" rel="noreferrer" href={artistObj.href} key={artistObj.id}>{`${artistObj.name}, `}</a>
            )
    })}, [spotifyObj]);
