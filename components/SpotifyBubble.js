...
import styles from  '../styles/components/SpotifyBubble.module.css';
    ...
    const mapArtists = (artists) => {
        return artists.map((artistObj, index) => {
            if (artists.length == 1 || (artists.length - 1) == index)
                return (
                    <div key={artistObj.id}>{artistObj.name}</div>
                )
    ...
    className={styles.spotify_container}>
        ...
        >
            <div className={styles.spotify_bubble_handle}>
                <FontAwesomeIcon icon={faGripLines} className={styles.drag_handle}/>
                <FontAwesomeIcon className={styles.spotify_bubble} icon={faSpotify}/>
                <FontAwesomeIcon icon={faGripLines} className={styles.drag_handle}/>
            </div>
            <div className={styles.spotify_bubble_content}>
                <Image className={styles.spotify_album_img} width={100} height={100} src={spotifyObj && spotifyObj.item ? spotifyObj?.item?.album?.images[0]?.url : cover}/>
                <div className={styles.spotify_data_container}>
                    <div className={styles.spotify_song_title}>
                        {spotifyObj?.item?.name ?? 'Not listening to anything right now!'}
                    </div>
