...
const BlogCard = ({height, opacity, x, index, post }) => {
  ...
  return (
    <Link href={`/blog/posts/${post.slug}`} style={{position: 'relative', width: '100%', border: '1px solid #101010', backgroundColor: "#101010", borderRadius: '10px'}}
    ...
    >
      ...
      <div className="mt-6 p-4 text-2xl font-bold">
        {post.title}
      </div>
      <div className="p-4 pt-2 text-lg">
        {post.excerpt}
      </div>
