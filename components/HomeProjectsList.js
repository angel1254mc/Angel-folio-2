...
const HomeProjectsList = ({dynamic = false, open = false, innerRef, projects = null}) => {
  ...
  return (
        ...
        {trail.map(({ height, opacity, x }, index) => (
            <ProjectCard key={projects[index].name + index} height={height} opacity={opacity} x={x} index={index} project={projects[index]}/>
        ))}
        </div>
  )
}
