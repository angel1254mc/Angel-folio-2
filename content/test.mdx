...
The Magnum Opus of all of Dan Abramov's work, ever. useRef that useStates. We have the pass our state around by reference, the state is maintained accross re-renders, AND we can trigger re-renders whenever we need to. This actually fixed our problem with ToneJS, but it is certainly not a sustainable fix. I'm not sure what goes on under the hood when React handles updating the values but I'm sure its not good for performance reasons, financial reasons, ethical reasons etc. Thought it would be something cool to showcase on the blog if anybody else is just finding this out 🤯. 
